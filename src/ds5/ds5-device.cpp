...
namespace librealsense
{
        ...
        const platform::backend_device_group& group)
    {
        ...
        dual_exposure de { exposure_range.def, gain_range.min, exposure_range.def, gain_range.def };
        ...
        auto ae_state_high = std::make_shared<auto_exposure_state>();
        ae_state_high->set_filter_mode(auto_exposure_state::filter_mode::high);
        auto auto_exposure_high = std::make_shared<auto_exposure_mechanism>(
            depth_sensor.get_option(RS2_OPTION_GAIN2), 
            depth_sensor.get_option(RS2_OPTION_EXPOSURE2), *ae_state_high);

        auto ae_state_low = std::make_shared<auto_exposure_state>();
        ae_state_low->set_filter_mode(auto_exposure_state::filter_mode::low);
        auto auto_exposure_low = std::make_shared<auto_exposure_mechanism>(
            depth_sensor.get_option(RS2_OPTION_GAIN1), 
            depth_sensor.get_option(RS2_OPTION_EXPOSURE1), *ae_state_low);

        auto auto_exposure_option_low = std::make_shared<enable_auto_exposure_option>(&depth_sensor,
                                                                                  auto_exposure_low,
                                                                                  ae_state_low,
                                                                                  option_range{0, 1, 1, 0});

        auto auto_exposure_option_high = std::make_shared<enable_auto_exposure_option>(&depth_sensor,
                                                                                  auto_exposure_high,
                                                                                  ae_state_high,
                                                                                  option_range{0, 1, 1, 0});
        ...
        depth_sensor.register_processing_block(
            ...
            [auto_exposure_option_low, auto_exposure_option_high]() {
                return std::make_shared<auto_exposure_processor>(RS2_STREAM_INFRARED, 
                    *auto_exposure_option_low, *auto_exposure_option_high);
            }
        );
